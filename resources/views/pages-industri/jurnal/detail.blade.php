    ...
    <div class="max-w-7xl mx-auto bg-white p-4 sm:p-6 rounded-lg shadow-md">
        ...
        <div class="mb-4">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center">
                <label for="dateFilter" class="mr-2 mb-2 sm:mb-0">Tanggal:</label>
                    ...
                    <tr data-tanggal="{{ \Carbon\Carbon::parse($item->tanggal)->format('Y-m-d') }}">
                        ...
                        <td class="py-2 px-4 border-b text-center">
                            @if ($item->foto_kegiatan)
                                <img src="{{ asset('storage/'.$item->foto_kegiatan) }}" alt="Foto Kegiatan" class="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded-full cursor-pointer" onclick="showActivityImage('{{ asset('storage/'.$item->foto_kegiatan) }}')">
                            @else
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mt-4">
            <span class="mb-2 sm:mb-0" id="pageNumber">Halaman {{ $listdetail->currentPage() }}</span>
            <div class="flex">
...
<script>
    ...
    let currentPage = 1;
    const rowsPerPage = 5; 
    const totalRows = {{ $listdetail->total() }};
